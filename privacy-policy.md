...
Swock is designed with a zero-data-collection philosophy. This privacy policy describes what
information Swock does and does not collect, how it uses permissions, and your rights regarding
your data.

**The short version: Swock collects no data. No network access, no analytics, no telemetry.
...
|:-----------|:-----|:----------|:--------|
| `BIND_ACCESSIBILITY_SERVICE` | System | Yes | Intercept touch gestures via `TouchInteractionController` |
...
- **What the service does**: Receives raw `MotionEvent` data via `TouchInteractionController`
  (API 33+) and inspects the accessibility node tree of the foreground app to classify whether
  the current screen is a Shorts/Reels player.
- **What the service does not do**: The service does not read, log, store, or transmit text
  content, credentials, personal data, or any other information from the accessibility node
  tree. Node tree inspection is used solely to determine screen type (Shorts feed vs. other
  screens) and is discarded immediately after classification.
- **Touch data**: Raw touch events are classified in real-time (swipe vs. tap vs. horizontal
  swipe) and then discarded. No touch data is stored or transmitted.
- **Scope**: The service only activates for apps the user has explicitly enabled in Swock's
  settings. It does not monitor apps the user has not selected.
...
Google's Play Integrity API is governed by [Google's Privacy Policy](https://policies.google.com/privacy).
The integrity token is requested on app launch and contains no personal information. Swock does
not currently have a backend server to verify the token — the request is made to Google's
services directly from the app.

## Data storage

All user preferences are stored locally on the device using Android's DataStore (Preferences
DataStore). This data never leaves the device.
